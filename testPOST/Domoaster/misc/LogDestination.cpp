...
/**@private*/
//DEFINE_s_int8(log_level, l, domoaster::DOMOASTER_LOG_WARN, "Set the logging level 0 .. 4.");
/**@private*/
...
log_level logging_level = DOMOASTER_LOG_WARN;
/**@endcond*/
...
void IncrementLogLevel() {
  ...
  if (logging_level == DOMOASTER_LOG_MAX)
    logging_level = DOMOASTER_LOG_NONE;
}
...
bool InitLoggingFromFlags() {
  ...
  if (true) { // FLAGS_syslog
    SyslogDestination *syslog_dest = new SyslogDestination(domoaster::DOMOASTER_LOG_NONE);
    if (!syslog_dest->Init()) {
      delete syslog_dest;
      return false;
    }
    ...
    destination = new StdErrorLogDestination(domoaster::DOMOASTER_LOG_WARN);
  }

  log_level log_level = domoaster::DOMOASTER_LOG_WARN;
  switch (4) { // FLAGS_log_level
    case 0:
      // nothing is written at this level
      // so this turns logging off
      log_level = domoaster::DOMOASTER_LOG_NONE;
      break;
    case 1:
      log_level = domoaster::DOMOASTER_LOG_FATAL;
      break;
    case 2:
      log_level = domoaster::DOMOASTER_LOG_WARN;
      break;
    case 3:
      log_level = domoaster::DOMOASTER_LOG_INFO;
      break;
    case 4:
      log_level = domoaster::DOMOASTER_LOG_DEBUG;
      break;
    default :
      break;
  }
...
bool InitLogging(log_level level, log_output output) {
  LogDestination *destination;
  if (output == DOMOASTER_LOG_SYSLOG) {
    ...
    destination = syslog_dest;
  } else if (output == DOMOASTER_LOG_STDERR) {
    destination = new StdErrorLogDestination(domoaster::DOMOASTER_LOG_WARN);
  } else {
    destination = NULL;
  }
...
void LogDestination::setLevel(log_level level){
	switch(level){
		case domoaster::DOMOASTER_LOG_INFO:
			this->_level = domoaster::DOMOASTER_LOG_INFO;
			break;
		case domoaster::DOMOASTER_LOG_FATAL:
			this->_level = domoaster::DOMOASTER_LOG_FATAL;
			break;
		case domoaster::DOMOASTER_LOG_WARN:
			this->_level = domoaster::DOMOASTER_LOG_WARN;
			break;
		case domoaster::DOMOASTER_LOG_DEBUG:
			this->_level = domoaster::DOMOASTER_LOG_DEBUG;
			break;
		default:
			this->_level = domoaster::DOMOASTER_LOG_NONE;
			break;
	}
}
/**
...
void SyslogDestination::Write(log_level level, const string &log_line) {
  int pri;
  switch (level) {
    case DOMOASTER_LOG_FATAL:
      pri = LOG_CRIT;
      break;
    case DOMOASTER_LOG_WARN:
      pri = LOG_WARNING;
      break;
    case DOMOASTER_LOG_INFO:
      pri = LOG_INFO;
      break;
    case DOMOASTER_LOG_DEBUG:
      pri = LOG_DEBUG;
      break;
    default :
      pri = LOG_INFO;
